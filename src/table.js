...
const CSS = {
	...
	wrapperWithAxisTitles: 'tc-wrap--with-axis-titles',
	table: 'tc-table',
	row: 'tc-row',
	withHeadings: 'tc-table--heading',
	withRowHeadings: 'tc-table--row-headings',
	withColumnHeadings: 'tc-table--column-headings',
	withBothHeadings: 'tc-table--both-headings',
	...
	automotiveMode: 'tc-table--automotive',
	axisTitle: 'tc-axis-title',
	axisTitleHorizontal: 'tc-axis-title--horizontal',
	axisTitleVertical: 'tc-axis-title--vertical',
	tableTitle: 'tc-table-title',
};
...
export default class Table {
	...
	constructor(readOnly, api, data, config) {
		...
		this.documentClicked = (event) => {
			const clickedInsideTable =
				event.target.closest(`.${CSS.table}`) !== null;
			const outsideTableClicked =
				event.target.closest(`.${CSS.wrapper}`) === null;
			const clickedOutsideToolboxes =
				clickedInsideTable || outsideTableClicked;
	...
	bindEvents() {
		...
		// Listen for cell content changes to update gradient colors
		this.table.addEventListener(
			'input',
			throttled(300, () => {
				if (this.tunes.gradientColors) {
	...
	addColumn(columnIndex = -1, setFocus = false) {
		...
		// Re-apply heading attributes after adding column
		if (
			this.tunes.withHeadings === 'row' ||
			this.tunes.withHeadings === 'both'
		) {
			this.addHeadingAttrToFirstRow();
		}
		if (
			this.tunes.withHeadings === 'column' ||
			this.tunes.withHeadings === 'both'
		) {
			this.addHeadingAttrToFirstColumn();
		}
	}
	...
	addRow(index = -1, setFocus = false) {
		...
		// Remove heading attributes before modifying structure
		if (
			this.tunes.withHeadings === 'row' ||
			this.tunes.withHeadings === 'both'
		) {
		...
		if (
			...
			this.removeHeadingAttrFromFirstColumn();
		}
		...
		// Re-apply heading attributes after modifying structure
		if (
			this.tunes.withHeadings === 'row' ||
			this.tunes.withHeadings === 'both'
		) {
	...
	deleteColumn(index) {
		...
		// Re-apply heading attributes after deleting column
		if (
			this.tunes.withHeadings === 'row' ||
			this.tunes.withHeadings === 'both'
		) {
	...
	deleteRow(index) {
		...
		// Re-apply heading attributes after deleting row
		if (
			this.tunes.withHeadings === 'row' ||
			this.tunes.withHeadings === 'both'
		) {
	...
	focusInTableListener(event) {
		...
		this.focusedCell = {
			row:
				Array.from(this.table.querySelectorAll(`.${CSS.row}`)).indexOf(row) +
				1,
			column:
	...
	/**
	 * Makes the first row and/or first column headings
	 *
	 * @param {string|false} withHeadings - heading configuration: false, 'row', 'column', or 'both'
	 */
	setHeadingsSetting(withHeadings) {
		this.tunes.withHeadings = withHeadings;

		// Remove all existing heading classes and attributes
		this.table.classList.remove(
			CSS.withHeadings,
			CSS.withRowHeadings,
			CSS.withColumnHeadings,
			CSS.withBothHeadings,
		);
		this.removeHeadingAttrFromFirstRow();
		this.removeHeadingAttrFromFirstColumn();

		// Handle different heading configurations
		if (withHeadings === 'row') {
			this.table.classList.add(CSS.withHeadings, CSS.withRowHeadings);
			this.addHeadingAttrToFirstRow();
		} else if (withHeadings === 'column') {
			this.table.classList.add(CSS.withHeadings, CSS.withColumnHeadings);
			this.addHeadingAttrToFirstColumn();
		} else if (withHeadings === 'both') {
	...
	/**
	 * Adds an attribute for displaying the placeholder in the first column cells
	 */
	addHeadingAttrToFirstColumn() {
		for (let rowIndex = 1; rowIndex <= this.numberOfRows; rowIndex++) {
	...
	/**
	 * Removes an attribute for displaying the placeholder in the first column cells
	 */
	removeHeadingAttrFromFirstColumn() {
		for (let rowIndex = 1; rowIndex <= this.numberOfRows; rowIndex++) {
			let cell = this.getCell(rowIndex, 1);
	...
	/**
	 * Enable or disable gradient colors
	 *
	 * @param {boolean} enabled - whether to enable gradient colors
	 */
	setGradientColors(enabled) {
		this.tunes.gradientColors = enabled;

		if (enabled) {
			this.table.classList.add(CSS.automotiveMode);
			this.applyGradientColors();
		} else {
			this.table.classList.remove(CSS.automotiveMode);
			this.clearGradientColors();
		}
	}

	/**
	 * Set color scheme for gradient colors
	 *
	 * @param {string} schemeName - name of color scheme (THERMAL, GRAYSCALE, VIRIDIS, AUTOMOTIVE)
	 */
	setColorScheme(schemeName) {
		this.tunes.colorScheme = schemeName;
	...
	/**
	 * Apply gradient colors to all cells based on their numeric values
	 */
	applyGradientColors() {
		const content = this.getData();
		const colorScheme =
			COLOR_SCHEMES[this.tunes.colorScheme] || COLOR_SCHEMES.THERMAL;

		// Determine what to skip based on skipHeadings setting and actual heading configuration
		const skipRow =
			this.tunes.skipHeadings &&
			(this.tunes.withHeadings === 'row' ||
				this.tunes.withHeadings === 'both');
		const skipCol =
			this.tunes.skipHeadings &&
		...
		const { min, max, hasValues } = getMinMaxValues(content, skipRow, skipCol);

		if (!hasValues) {
			return;
		}

		const startRow = skipRow ? 2 : 1;
		const startCol = skipCol ? 2 : 1;
		...
		for (let i = 1; i <= this.numberOfRows; i++) {
			for (let j = 1; j <= this.numberOfColumns; j++) {
				...
				// Skip if we're in the heading row/column
				...
				const value = parseNumericValue(cell.innerHTML);
	...
	/**
	 * Clear gradient colors from all cells
	 */
	clearGradientColors() {
		for (let i = 1; i <= this.numberOfRows; i++) {
			for (let j = 1; j <= this.numberOfColumns; j++) {
	...
	/**
	 * Update table title (only creates if shown)
	 *
	 * IMPORTANT: Do NOT create contentEditable elements in the DOM unless they are actively shown.
	 * Editor.js positions the tune menu buttons by finding the first contentEditable element in the block.
	 * If we create a hidden contentEditable element (even with display:none), Editor.js will still find it
	 * and use it for positioning, causing the tune menu buttons to render at the wrong location
	 * (typically at the top of the screen instead of next to the actual content).
	 */
	updateTableTitle(show = false) {
		// Remove existing title
		const existingTitle = this.wrapper.querySelector(`.${CSS.tableTitle}`);
		if (existingTitle) {
		...
		const tableTitle = $.make('div', [CSS.tableTitle], {
			contentEditable: !this.readOnly,
			innerHTML: this.tunes.tableTitle || '',
		});

		tableTitle.setAttribute('data-placeholder', 'Table title...');

		tableTitle.addEventListener('blur', (e) => {
			this.tunes.tableTitle = e.target.innerHTML;
		});
		...
		// Insert at the very beginning of the wrapper
		this.wrapper.insertBefore(tableTitle, this.wrapper.firstChild);
	}

	/**
	 * Toggle table title visibility
	 *
	 * @param {boolean} show - whether to show table title
	 */
	toggleTableTitle(show) {
		if (show) {
			// Create and show the title
			this.updateTableTitle(true);
		} else {
			// Remove the title completely
			const tableTitle = this.wrapper.querySelector(`.${CSS.tableTitle}`);
			if (tableTitle) {
				tableTitle.remove();
			}
		}
	 ...
	 *
	 * @param {string} title - horizontal axis title
	 */
	setHorizontalTitle(title) {
		this.tunes.horizontalTitle = title;
		// Update if currently visible
		const hTitle = this.wrapper.querySelector(`.${CSS.axisTitleHorizontal}`);
		if (hTitle) {
	 ...
	 *
	 * @param {string} title - vertical axis title
	 */
	setVerticalTitle(title) {
		this.tunes.verticalTitle = title;
		// Update if currently visible
		const vTitle = this.wrapper.querySelector(`.${CSS.axisTitleVertical}`);
		if (vTitle) {
	...
	/**
	 * Update axis title elements in the DOM (only creates if shown)
	 *
	 * IMPORTANT: Do NOT create contentEditable elements in the DOM unless they are actively shown.
	 * Editor.js positions the tune menu buttons by finding the first contentEditable element in the block.
	 * If we create a hidden contentEditable element (even with display:none), Editor.js will still find it
	 ...
	 * (typically at the top of the screen instead of next to the actual content).
	 */
	updateAxisTitles(show = false) {
		// Remove existing titles
		const existingTitles = this.wrapper.querySelectorAll(`.${CSS.axisTitle}`);
		existingTitles.forEach((title) => title.remove());

		// Only create if we need to show them
		if (!show) {
			return;
		}

		// Always add horizontal title field
		const hTitle = $.make('div', [CSS.axisTitle, CSS.axisTitleHorizontal], {
			contentEditable: !this.readOnly,
			innerHTML: this.tunes.horizontalTitle || '',
		});

		hTitle.setAttribute('data-placeholder', 'Horizontal axis title...');

		hTitle.addEventListener('blur', (e) => {
			this.tunes.horizontalTitle = e.target.innerHTML;
		});

		hTitle.addEventListener('keydown', (e) => {
			if (e.key === 'Enter') {
				e.preventDefault();
				e.target.blur();
			}
		...
		this.wrapper.insertBefore(hTitle, this.wrapper.firstChild);

		// Always add vertical title field
		const vTitle = $.make('div', [CSS.axisTitle, CSS.axisTitleVertical], {
			contentEditable: !this.readOnly,
			innerHTML: this.tunes.verticalTitle || '',
		});

		vTitle.setAttribute('data-placeholder', 'Vertical axis title...');

		vTitle.addEventListener('blur', (e) => {
			this.tunes.verticalTitle = e.target.innerHTML;
		});
		...
		this.wrapper.appendChild(vTitle);
	}

	/**
	 * Toggle axis titles visibility
	 *
	 * @param {boolean} show - whether to show axis titles
	 */
	toggleAxisTitles(show) {
		if (show) {
			// Create and show the titles
			this.updateAxisTitles(true);
			this.wrapper.classList.add(CSS.wrapperWithAxisTitles);
		} else {
			// Remove the titles completely
			const existingTitles = this.wrapper.querySelectorAll(
				`.${CSS.axisTitle}`,
			);
			existingTitles.forEach((title) => title.remove());
			this.wrapper.classList.remove(CSS.wrapperWithAxisTitles);
		}
	}
